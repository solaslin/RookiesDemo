...
{		
    ...
    vec4 baseColor = texture2D(colorMap,uvCoord) * mtrColor;
    vec3 litColor = baseColor.rgb * _ROOKIES_MainLightColor * diff;
    ...
    reflUV += 0.5;
    //reflUV = vNormalVec.xy * 0.5 + 0.5;     // temp
    ...
    gl_FragData[0] = vec4(mix(litColor, reflColor, factor), baseColor.a);
}
